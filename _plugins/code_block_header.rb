...
module CodeBlockHeader
  # Inject code-block headers into an HTML string.
  # Returns the modified HTML string (always re-serialised when any highlight
  # block is present, so IDs and headers are always injected consistently).
...
Jekyll::Hooks.register [:pages, :documents], :post_render do |doc|
  ...
  doc.output = CodeBlockHeader.transform(doc.output)
end
