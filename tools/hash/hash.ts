// ── Compact MD5 (Paul Johnston, public domain) ────────────────────────────
...
export function md5(str: string): string {
  ...
  function binl2hex(arr: number[]): string {
    const hex = '0123456789abcdef'
    let out = ''
    for (let i = 0; i < arr.length * 4; i++) {
      out +=
        hex[(arr[i >> 2]! >> ((i % 4) * 8 + 4)) & 0xf] +
        hex[(arr[i >> 2]! >> ((i % 4) * 8)) & 0xf]
    }
    return out
  }

  // Encode to UTF-8 bytes first (handles non-ASCII characters)
...
export function sha(algo: AlgorithmIdentifier, text: string): Promise<string> {
