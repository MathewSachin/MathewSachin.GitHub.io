...
The original <PostLink post={getPostByUrl('/blog/2026/03/21/youtube-shorts-sniper')} /> introduced the Delta-Check to detect a loop restart — because Shorts loop by default and never fire `ended`. The <PostLink post={getPostByUrl('/blog/2026/03/21/instagram-reel-sniper')} /> used the simpler `video.onended`, because Reels play exactly once and then stop cleanly.
...
*Using the same browser-only approach, you can also <PostLink post={getPostByUrl('/blog/2026/03/22/reddit-video-sniper')} />, <PostLink post={getPostByUrl('/blog/2026/03/21/instagram-reel-sniper')} />, <PostLink post={getPostByUrl('/blog/2026/03/21/youtube-shorts-sniper')} />, and <PostLink post={getPostByUrl('/blog/2026/03/21/save-instagram-photos')} />.*
