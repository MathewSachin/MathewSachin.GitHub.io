...
description: "Apply the Dino hacks on your phone — no computer, no DevTools. Teaches: bookmarklets, URL encoding."
...
The <PostLink post={getPostByUrl('/blog/2016/11/05/chrome-dino-hack')} /> rely on the Chrome Developer Console to run JavaScript. On a desktop that's one keyboard shortcut away, but on a phone or tablet the console simply isn't available. That's where **bookmarklets** come in.
...
Want to go further? Check out the full <PostLink post={getPostByUrl('/blog/2016/11/05/chrome-dino-hack')} /> for speed control, score manipulation, auto-play, and more tricks you can use on desktop!
