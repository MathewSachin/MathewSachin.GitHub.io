...
The `groundYPos` property controls the vertical position where the dino “rests” when not jumping — measured in pixels from the top of the canvas.
The normal ground level is **93**.
Setting it to `0` moves the dino’s resting position to the very top of the screen.

Use the slider to position the dino anywhere from the sky (`0`) to the normal ground level (`93`).
Intermediate values like `40` or `60` let you glide over ground-level cacti without getting hit.
Obstacles still scroll past at fixed heights, so this is a sneaky alternative to God Mode!
