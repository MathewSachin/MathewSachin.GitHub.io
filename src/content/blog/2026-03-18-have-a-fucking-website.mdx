...
import haveAWebsiteHero from '@images/have-a-website-hero.svg';
...
<Picture src={haveAWebsiteHero} alt="Visual contrast: social media platform icons labeled rented land with locks on the left, versus a personal website browser mockup labeled yours forever on the right" />
...
This blog exists precisely because of the dynamic the post describes. <PostLink post={getPostByUrl('/blog/2026/03/12/ai-blog-generation-flow')} /> — content published to a domain I control, with no algorithm deciding who sees it, no platform that can shadow-ban a post or change the rules about links.
