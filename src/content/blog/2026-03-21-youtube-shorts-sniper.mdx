...
This is a fundamentally different approach from the <PostLink slug="2026-3-21-instagram-reel-sniper" />, which has to loop through every `<video>` on the page and check each one's bounding rectangle to find the visible one. YouTube gives us a cleaner semantic handle with `is-active`.
...
For a full breakdown of the `captureStream` + `MediaRecorder` pattern, see the <PostLink slug="2026-3-21-instagram-reel-sniper" /> post. The core pipeline is identical. Here's where the two scripts diverge:
...
*Using the same browser-only approach, you can also <PostLink slug="2026-3-22-reddit-video-sniper" />, <PostLink slug="2026-3-21-instagram-reel-sniper" />, <PostLink slug="2026-3-21-twitter-x-video-sniper" />, and <PostLink slug="2026-3-21-save-instagram-photos" />.*
